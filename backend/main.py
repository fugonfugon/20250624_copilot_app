...
async def explain_code(data: dict):
    try:
        ...
        line_index = data.get("line_index", -1)  # フロントエンドから取得

        if not selected:
            return {"explanation": "コードが空です。"}

        # 文脈抽出：前後3セル
        start = max(0, cell_index - 3)
        end = min(len(all_cells), cell_index + 4)
        context_cells = all_cells[start:end]

        # 選択セルのハイライト付き表示
        highlight_cell = all_cells[cell_index]
        highlighted = highlight_cell.replace(selected, f"<<{selected}>>")

        prompt = (
            "以下はあるPythonノートブックの一部です。\n"
            "「# WRITE ME」という場所に何を書けばよいか、選択されたコードの周囲の文脈を考慮して、"
            "初学者に向けて**ヒントのみ**を日本語でやさしく提示してください。\n"
            "選択された部分は `<< >>` で囲んで示されています。\n\n"
            f"【選択されたコードセル（セル{cell_index + 1}）】\n{highlighted}\n\n"
            f"【このセルの前後3コードセル分の文脈】\n{'\n\n'.join(context_cells)}"
        )
