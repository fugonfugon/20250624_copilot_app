...
const handleMouseUp = () => {
  ...
  const lineIndex = cells[targetIndex]
    .split("\n")
    .findIndex(line => line.includes("# WRITE ME") && line.includes(selected));
...
const runCell = async (code, index) => {
        ...
        <div className="p-6 overflow-auto bg-gray-50" onMouseUp={handleMouseUp}>
          ...
          {cells.map((cell, idx) => (
            <div key={idx} className="bg-white rounded-lg shadow p-4 mb-6">
              ...
              />
                <div className="mt-2">
                <button onClick={() => runCell(cell, idx)} className="px-3 py-1 bg-purple-500 text-white rounded hover:bg-purple-600">▶ 実行</button>
              </div>
              {outputLog[idx] && (
                <div className="mt-2 p-2 bg-gray-100 border rounded text-sm font-mono whitespace-pre-wrap">{outputLog[idx]}</div>
              )}
            </div>
          ))}
        </div>
